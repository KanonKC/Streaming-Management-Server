...
describe('RevealTarotCard', () => {
    describe('Basic functionality', () => {
        it('Should return a random Tarot card', async () => {
            const response = await revealTarotCard()
            expect(response).toHaveProperty('majorCard')
            expect(response).toHaveProperty('minorCard')
        })
        it('Should return a specific Major Tarot card', async () => {
            const response = await revealTarotCard(0)
            expect(response.majorCard.id).toBe(0)
    
            const response2 = await revealTarotCard(5)
            expect(response2.majorCard.id).toBe(5)
    
            const response3 = await revealTarotCard(21)
            expect(response3.majorCard.id).toBe(21)
        })
        it('Should return a specific Minor Tarot card', async () => {
            const response = await revealTarotCard(0, 22)
            expect(response.majorCard.id).toBe(0)
            expect(response.minorCard.id).toBe(22)
    
            const response2 = await revealTarotCard(5, 30)
            expect(response2.majorCard.id).toBe(5)
            expect(response2.minorCard.id).toBe(30)
    
            const response3 = await revealTarotCard(21, 77)
            expect(response3.majorCard.id).toBe(21)
            expect(response3.minorCard.id).toBe(77)
        })
    })
    describe('Handle error', () => {
        it.skip('Should return a random Tarot card if majorCardId is out of bounds', async () => {
            const response = await revealTarotCard(-1)
            expect(response.majorCard.id).toBeGreaterThanOrEqual(0)
            expect(response.majorCard.id).toBeLessThan(22)
        })
        it.skip('Should return a random Tarot card if minorCardId is out of bounds', async () => {
            const response = await revealTarotCard(0, -1)
